...
import { useOverview, useAvailableYears } from "@/hooks/useKpiData";
...
const STATUS_BADGE: Record<string, string> = {
  ...
  yellow: "bg-blue-100 text-blue-700 border-0",
  red: "bg-blue-100 text-blue-700 border-0",
};
...
export default function Overview() {
  ...
  const { data, isLoading, error } = useOverview(year);
  ...
  if (error || !data) {
    return (
      <div className="flex items-center gap-3 text-destructive p-4">
        ...
        <span className="text-sm">Failed to load overview data. Make sure the backend is running on port 8000.</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            KPI performance monitoring · {year}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {summaryCards.map((c) => (
          <Card key={c.label} className="p-3 md:p-4 flex items-center gap-3 shadow-sm border-0 shadow-foreground/5">
            <div className={`w-9 h-9 md:w-10 md:h-10 rounded-lg flex items-center justify-center shrink-0 ${c.accent}`}>
            ...
            <div className="min-w-0">
              ...
              <p className="text-lg md:text-xl font-bold tracking-tight">{c.value}</p>
            </div>
          </Card>
        ))}
      </div>
        ...
        <Card className="p-4 md:p-5 shadow-sm border-0 shadow-foreground/5">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={barData}>
              ...
              <YAxis domain={[0, 120]} tick={{ fontSize: 10 }} width={30} />
              <Tooltip
      ...
      {/* Division rankings — horizontal scroll on mobile */}
      <Card className="p-4 md:p-5 shadow-sm border-0 shadow-foreground/5">
